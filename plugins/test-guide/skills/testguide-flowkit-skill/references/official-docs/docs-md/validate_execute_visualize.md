...
Validate, execute and visualize your flow definition [ Validate, execute and visualize your flow definition ](validate_execute_visualize.md)

Table of contents

- [ Validate flow bundle ](#validate-flow-bundle)
  - [ Unit tests ](#unit-tests)
- [ Execute flow bundle locally ](#execute-flow-bundle-locally)
- [ Visualize flow definition (experimental feature) ](#visualize-flow-definition-experimental-feature)
...
The flow bundle contains run configurations with which the implemented flow definition can be validated, executed and visualized. These run configurations are part of the automatic [IDE setup](setup_ide.md) included in the flow.kit.

## Validate flow bundle

The run configuration **validate flow definition** is a check for your flow definition. It includes the following checks:

- ensuring that the flow definition can be parsed successfully
- check that the necessary parameters for all blocks are set
- check that given parameter values match with the wanted parameter type
- run unit tests defined in `test_user_code.py` (see [Unit tests](validate_execute_visualize.md#unit-tests))

Tip

**Execute validation after every change** to the flow definition in order to detect static errors.

### Unit tests

You can define unit tests for your flow definition, esp. for the user-defined functions, in the file `test_user_code.py` next to your `flow_definition.py`. Methods in this file with the name prefix `test_` will be treated as unit tests and will be executed with pytest during validation.

## Execute flow bundle locally

The run configuration **execute flow definition** runs the flow bundle on your local machine to get fast feedback during development and debugging of the flow definition.

The payload for the flow bundle execution must be defined in the file `payload.json` next to your `flow_definition.py`.

For execution, the necessary environment variables must be created or adapted in a `.env` file in the root of the flow bundle. The `.env` file is not included in the flow bundle by default, but you can create it yourself using the provided `.env.example` file as a template.

If additional environment variables are required for the flow bundle, these must also be added in the `.env` file.

Don't commit the `.env` file!

The `.env` file may contain sensitive information, such as API keys or passwords. It is added to the `.gitignore` file by default. Don't change this!

## Visualize flow definition (experimental feature)

The run configuration **visualize flow definition** provides a simple way to generate a graphical overview of the implemented flow definition. It generates a \*.puml file that can be rendered with PlantUML.

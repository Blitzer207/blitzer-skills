...
Proxy configuration [ Proxy configuration ](proxy.md)

Table of contents

- [ Supported environment variables ](#supported-environment-variables)
- [ Usage ](#usage)
  - [ IDE - run configuration ](#ide-run-configuration)
  - [ Examples ](#examples)
...
This guide explains how to configure proxy settings for flow.kit using environment variables, following the conventions supported by urllib3.

Restrictions

Only the blocks of **Codebeamer, ETM, Jama, and Polarion** will respect these settings.

## Supported environment variables

The following environment variables should be set before executing the flow.

- `HTTP_PROXY` / `http_proxy`: Used for HTTP requests
- `HTTPS_PROXY` / `https_proxy`: Used for HTTPS requests
- `NO_PROXY` / `no_proxy`: Comma-separated list of hosts or domains to exclude from proxy

## Usage

### IDE - run configuration

You can set environment variables for the run configuration **execute flow definition** inside your IDE mentioned in [Validate, execute and visualize your flow definition](validate_execute_visualize.md#execute-flow-bundle-locally).

### Examples

    # Set proxy for urls with https schema
    HTTPS_PROXY='http://myproxy.local'

    # Set proxy for urls with http schema
    HTTP_PROXY='http://myproxy.local'

    # Proxy with username
    HTTPS_PROXY='http://username@myproxy.local'

    # Proxy with username and password
    HTTPS_PROXY='http://username:p4s$w0rD@myproxy.local'

    # Exclude all urls from proxy usage
    # Note: local wildcards like '*domain.com' are not supported  
    NO_PROXY='*'

    # Exclude specific urls from proxy usage
    # Note: value is case-insensitive
    NO_PROXY='localhost,mylocalserver.local,mycompany'

    # Lowercase variables take precedence
    HTTPS_PROXY='proxy.not.to.be.used.local'
    https_proxy='http://myproxy.local'  # this will be used

    HTTP_PROXY='proxy.not.to.be.used.local'
    http_proxy='http://myproxy.local'  # this will be used

    NO_PROXY='url.failed.to.ignore.local'
    no_proxy='url.ignored.local'  # this will be used
